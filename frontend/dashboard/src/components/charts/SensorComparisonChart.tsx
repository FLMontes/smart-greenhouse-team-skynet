...
export function SensorComparisonChart(props: SensorComparisonChartProps) {
  ...
  // Transform temperature data for comparison - group by sensor
  const chartData = data.length > 0
    ? data
        .reduce(
          (acc: Record<string, any>[], reading) => {
            const time = new Date(reading.createdAt).toLocaleTimeString('es-AR', {
              hour: '2-digit',
              minute: '2-digit',
            });
            ...
            entry[reading.sensorId] = reading.temperature;
            return acc;
          },
          []
        )
  ...
  return (
    <div className="rounded-lg border border-gray-200 bg-white p-6 shadow-sm">
      ...
      <p className="text-sm text-gray-500 mb-4">Comparativa de temperatura por sensor</p>
